 ...
 */
/*
 * 对客服端的cookie进行处理，return
 * */
function getCookie($param=array(),$cookieKey=array('JSESSIONID')){
    $cooks=array();
    $cook='';
    foreach($cookieKey as $key=>$value){
        if(isset($_COOKIE[$value])){
            $cooks[$value]=$_COOKIE[$value];
        }
    }
    $cooks=$cooks+$param;
    foreach($cooks as $key=>$va){
        if(isset($va)){
            $cook=$cook.$key.'='.$va.';';
        }
    }
 ...
 * @return mixed
 */
 function curl_post_contents($durl, $data = array(),$head=array(),$cook=array())
{
    ...
    curl_setopt($ch, CURLOPT_COOKIE, getCookie($cook));
 ...
 * @return mixed
 */
function curl_get_contents($durl, $data = array(),$head=array(),$cook=array())
{
    if (isset($_GET['debug']) && $_GET['debug'] == DEBUG_KEY) {
        echo '请求接口：' . $durl . '<br>';
    }
