 ...
 * */
    ...
    *初始化smarty
    ...
    public static function setSmartyVar($response)
    {
        ...
        $GLOBALS['smarty']->assign('evnCfg', $GLOBALS['evnCfg']);
    ...
    public static function rolling_curl($cfgs)
    {
        ...
        foreach ($cfgs as $item => $value) {
            ...
            foreach ($params as $key => $val) {
                ...
                curl_setopt($ch, CURLOPT_COOKIE, getCookie());
     ...
     * @return mixed
     */
    public static function curl_post_contents($durl, $data = array(),$head=array(),$cook=array())
    {
        if (isset($_GET['debug']) && $_GET['debug'] == DEBUG_KEY) {
            echo '请求接口：' . $durl . '<br>';
        }
     ...
     * @return mixed
     */
    public static function curl_get_contents($durl, $data = array(),$head=array(),$cook=array())
    {
        if (isset($_GET['debug']) && $_GET['debug'] == DEBUG_KEY) {
            echo '请求接口：' . $durl . '<br>';
        }
