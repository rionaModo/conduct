<script>
    ...
    {if $smarty.const.STATIC_FROM=='yes'}
        ...
        SYS_JS_COURCE.push('{$STATIC_PRE|cat:$item|regex_replace:'/\.js/':'.min.js'}?{file_createtime path='/danlu.com/dlmain'|cat:$item|regex_replace:'/\.js/':'.min.js'}');
        {/foreach}
    {else}
        {foreach from=$js_config  key=key item=item}
        SYS_JS_COURCE.push('{$STATIC_PRE|cat:$item}?{file_createtime path='/danlu.com'|cat:$STATIC_PRE|cat:$item}');
        {/foreach}
    {/if}
