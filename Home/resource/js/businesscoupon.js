...
var query=urlParse();
var defaultImg=STATIC_PRE+"/resource/images/businesscoupon/default.jpg";
...
function create_dealer(obj){//立即领取
    ...
    $.ajax({
        type: "POST",
        url: "/router/dealer/create_dealer_trading",
        ...
        success: function(data){
            ...
            if(type==2){
                if(data.status!=2&&data.status!=3&&data.status!=8 && data.status!=7){
                    data.status=3;
            ...
            data.type=type;
            if(data.status==3||data.status==1||data.status==4 ||data.status==5){
                if(data.status==5){
...
function openLayerHandle(data,dom,index){//立即领取请求成功处理函数
    if(data.status != 4 && data.status != 5){
        ...
        if(data.status==0||data.status==2){
            ...
            // if(data.status==2){
            if(data.status==2 && data.data.remainNum == 1){
                list.find('.clayer-right > .clayer-right-do').append('<p class="clayer-right-do" style="color:#fc2e3a;">(您的可领优惠机会已用完)</p>');
            }
        }
        ...
        if(data.status == 4 ){
            var indexFlag = layer.open({
              title:" ",
              skin:'dealer-benefit-skin',
              type: 1,
              closeBtn: 1, //不显示关闭按钮
              shadeClose: false, //开启遮罩关闭
              content: '<p class="benefit-process">您好，抢券结果正在处理中！请等待处理完毕后再领取！</p>',
              btn: ['确认'],
              yes: function(index, layero){
                layer.closeAll();
              }
            });
            layer.style(indexFlag, {
            ...
            var indexFlag = layer.open({
              ...
              closeBtn: 1, //不显示关闭按钮
              shadeClose: false, //开启遮罩关闭
              content: '<p class="benefit-process">您的可领取优惠机会已用完</p>',
              btn: ['确认'],
              yes: function(index, layero){
                layer.closeAll();
              }
...
function addPackets(v,type){
    // dealerCouponStatus (string, optional),优惠券状态：可领取:01 ，已领取:02，已领完:03，已过期:04
    var liClass='';
...
function addContent(qs,selector,type){
    ...
    $.ajax({
        type:'POST',
        url: '/router/dealerCoupon/getCenterCouponDetail',
...
(function(){
    ...
    addChangeEvent($("#shopName")[0],function(){
        ...
        associate({
            url:"/router/dealerCoupon/nameMatching",
            target:"#shopNameD",
    ...
    addChangeEvent($("#goodsName")[0],function(){
        ...
        associate({
            url:"/router/dealerCoupon/goodsMatching",
            target:"#goodsNameD",
